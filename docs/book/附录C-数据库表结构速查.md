...
| 分类 | 表数量 | 表名 |
|------|--------|------|
| 核心业务表 | 6 | `users`、`servers`、`server_groups`、`server_group_mapping`、`agents`、`workflows` |
| 任务与执行表 | 5 | `tasks`、`agent_executions`、`scheduled_tasks`、`server_command_history`、`compliance_checks` |
| 告警与通知表 | 8 | `alerts`、`alert_configs`、`alert_notifications`、`alert_workflow_mappings`、`alert_noise_reduction`、`alert_webhook_logs`、`notifications`、`notification_config` |
| 自愈与根因分析表 | 5 | `remediation_policies`、`remediation_executions`、`remediation_history`、`remediation_cooldowns`、`root_cause_analyses` |
| 知识与脚本表 | 2 | `knowledge_base`、`scripts` |
| 报告表 | 2 | `reports`、`report_schedules` |
| 运维与安全表 | 4 | `audit_logs`、`settings`、`token_blacklist`、`encryption_keys` |
| 监控与备份表 | 2 | `server_metrics`、`backups`（逻辑表，实际存储于文件系统） |
| 辅助表 | 1 | `copilot_conversations` |
